...
extern int WifiGetRssiAsQuality(int q);
extern int32_t restart_counter;
extern char lastStart[1024];

/**************************************************************************************************
 *    Function      : update_clients
 *    Description   : Parses POST
 **************************************************************************************************/
void update_clients()
{
  server->send(200);
...
/**************************************************************************************************
 *    Function      : response_clients
 *    Remarks       : Retunrs the notes as plain text
 **************************************************************************************************/
void read_clients()
{
  const size_t capacity = 2048; // maybe less
  StaticJsonDocument<capacity> doc;
  String response = "";

  for (int i = 0; i < totalClients; i++)
  {
    doc[i]["IP"] = ntp_clients[i].clientIP;
    doc[i]["syncs"] = ntp_clients[i].syncs;
    const time_t t = ntp_clients[i].last_stamp;
    struct tm *ti;
    ti = localtime(&t);
    char buf[100];
    sprintf(buf, "%02d.%02d.%04d  %02d:%02d:%02d", ti->tm_mday, ti->tm_mon, ti->tm_year + 1900, ti->tm_hour, ti->tm_min, ti->tm_sec);
 ...
 **************************************************************************************************/
void settime_update()
{ /* needs to process date and time */
  datum_t d;
  d.year = 2000;
  d.month = 1;
  d.day = 1;
  ...
  if (!server->hasArg("date") || server->arg("date") == NULL)
  ...
  if (!server->hasArg("time") || server->arg("time") == NULL)
  { // If the POST request doesn't have username and password data
  }
  else
  {
 ...
 *    Remarks       : none
 **************************************************************************************************/
void timezone_update()
{ /*needs to handel timezoneid */
  if (!server->hasArg("timezoneid") || server->arg("timezoneid") == NULL)
  { // If the POST request doesn't have username and password data
    /* we are missong something here */
  }
  else
 ...
 *    Remarks       : none
 **************************************************************************************************/
void timezone_overrides_update()
{ /* needs to handle DLSOverrid,  ManualDLS, dls_offset, ZONE_OVERRRIDE and GMT_OFFSET */
  ...
  if (!server->hasArg("dlsdis") || server->arg("dlsdis") == NULL)
  { // If the POST request doesn't have username and password data
  ...
  if (!server->hasArg("dlsmanena") || server->arg("dlsmanena") == NULL)
  { // If the POST request doesn't have username and password data
    /* we are missing something here */
  }
  else
  ...
  if (!server->hasArg("ZONE_OVERRRIDE") || server->arg("ZONE_OVERRRIDE") == NULL)
  { // If the POST request doesn't have username and password data
    /* we are missing something here */
  }
  else
  ...
  if (!server->hasArg("gmtoffset") || server->arg("gmtoffset") == NULL)
  { // If the POST request doesn't have username and password data
    /* we are missing something here */
  }
  else
  ...
  if (!server->hasArg("dlsmanidx") || server->arg("dlsmanidx") == NULL)
  { // If the POST request doesn't have username and password data
    /* we are missing something here */
  }
  else
...
void update_notes()
{
  ...
  if (!server->hasArg("notes") || server->arg("notes") == NULL)
  { // If the POST request doesn't have username and password data
    /* we are missing something here */
  }
  else
...
void read_notes()
{
  ...
  sprintf(data, "Total UpTime Days: %03d T: %02d:%02d:%02d \nFree Heap: %d kB \nWifi Quality: %d% % %d dB\nRestart: %s"
  , ui.day, ui.hour, ui.minute, ui.second
  ,ESP.getFreeHeap()/1024
  , wifiq, WiFi.RSSI()
  ,lastStart);
  // snprintf(data, 501, "Debug info:\n%s");
...
void getGPS_Location()
{
  ...
  root["lon"] = gps.location.lng();
  root["alti"] = gps.altitude.meters();
  root["hdop"] = gps.hdop.value() / 100;
...
{
  if (server->arg("GPS_SYNC_ON") == "false")
  {
    /* we are missing something here */
  ...
  Serial.printf("sync_on_gps : %s \n", gps_config.sync_on_gps ? "true" : "false");
...
void update_ipv4_settings(void)
{
  ...
  if (!server->hasArg("JSON") || server->arg("JSON") == NULL)
  { // If the POST request doesn't have username and password data
    /* we are missing something here */
  ...
  else
  {
    ...
    if (server->arg("JSON").length() < 2048)
    { /* we only process messages up to 2048 elements */
      DynamicJsonDocument doc(2048);
      DeserializationError dErr = deserializeJson(doc, server->arg("JSON"));
...
void update_display_settings(void)
{
  ...
  // show screen no. > display
  uint8_t display = (uint8_t)server->arg("display").toInt();

  if (display == 1) // prev
  {
    if (disp_config.display_info <= 1)
      disp_config.display_info = getFrameCount();
    else
      disp_config.display_info--;
    dsiplDirLeft = false;
  }
  else if (display == 2) // next
  {
    if (disp_config.display_info >= getFrameCount())
      disp_config.display_info = 1;
    else
      disp_config.display_info++;
    dsiplDirLeft = true;
  }
  else if (display == 3) // next
  {
    if (server->arg("showNmea") == "true")
    {
      /* Enable Swap */
      disp_config.show_nmea = true;
    }
    else
    {
      disp_config.show_nmea = false;
    }
  }

  write_display_config(disp_config);
  server->send(200);
  displayconfig = read_display_config(); // set global
  // send_display_settings();
  // Serial.printf("i: %d sw: %d nmea: %d\n", disp_config.display_info, disp_config.swap_display, disp_config.show_nmea);
}
...
void send_display_settings(void)
{
  ...
  root["display_info"] = disp_config.display_info;
  root["show_nmea"] = disp_config.show_nmea;
  root["gps_sync"] = gps_config.sync_on_gps;

  root["host"] = m_wifiHostname.c_str(); // crd.hostname;
