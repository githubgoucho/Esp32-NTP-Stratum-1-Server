...
but verrry bad calculated ... */
#define CREDENTIALS_START 0
/* credentials are 256+128+ crc 4Byte */
#define TIMECORECONFIG_START 500
/* config is 24 byte + crc 4 byte */

#define GPSCONFIG_START 540
/* config is 4 byte + 4 byte */

#define DISPLAYCONFIG_START 620

#define IPV4SETTINGS_START 640

#define NOTES_START 720
/* notes take 512 byte */
...
display_settings_t read_display_config(void)
{
  ...
  {
    Serial.println("EERead DISPLAY CONF failed");
...
timecoreconf_t read_timecoreconf(void)
{
  ...
  {
    Serial.println("EERead TIME CONF failed");
...
gps_settings_t read_gps_config(void)
{
  ...
  {
    Serial.println("EERead GPS failed");
 ...
 *    Remarks       : if fails, struct & eeprom is set to 0
 **************************************************************************************************/
credentials_t read_credentials(void)
{
  ...
  {
    // set to 0
    Serial.println("EERead cred failed");
...
ipv4_settings read_ipv4_settings(void)
{
  ...
  {
    Serial.println("EERead IPv4 failed");
...
bool eepread_struct(void *element, uint32_t e_size, uint32_t startaddr)
{
  ...
  uint8_t data;
#ifdef DBG
  Serial.printf("start %x size %x - data ", e_size, startaddr);
#endif
  ...
  {
    Serial.println("SAVED CONF failed");
    done = false;
  }
