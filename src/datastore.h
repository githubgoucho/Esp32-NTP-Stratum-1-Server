...
typedef struct {
  IPAddress clientIP; // 4 byte
  uint32_t last_stamp; // 4 byte ( seconds since 1.1.1970)
  uint16_t syncs;  // 2 byte
} client_settings_t;
...
void eepwrite_clients(uint8_t* data, uint32_t size);

void eepread_clients(uint8_t* data, uint32_t size);
